...
with TopDonors as (
	select partyName, purchaser, Amount, Tx, dense_rank () OVER (PARTITION BY partyName,purchaser ORDER BY Amount DESC) AS topDonor 
	from (
	...
	partyName, purchaser, Amount, Tx, topDonor
 from
 	TopDonors
  where 
     topDonor <= 11;
